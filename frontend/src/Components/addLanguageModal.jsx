...
const AddLanguageModal = ({
  ...
  const [isProto, setIsProto] = useState(false);
  ...
  const save = async () => {
    ...
    //ensure that language details inside addedGroups array is up to date before saving (i.e if user changed anything after assigning groups)
    addedGroups.forEach((group) => {
      group.languages.forEach((language) => {
        if (language.language_id === idRef.current) {
          language.word_forms = wordForms;
          language.language_name = languageName;
          language.mother_language_id = selectedParentLanguage
  ...
  const handleGroupSelect = (group) => {
    ...
    const language = {
      ...
      language_id: idRef.current,
      language_name: languageName,
      mother_language_id: selectedParentLanguage
        ? selectedParentLanguage.language_id
        : null,
    };
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>{translate("Add New Language")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
              ...
              <div className="thin-white-border">
                <div style={{ display: "flex", flexDirection: "row" }}>
                  <label style={{ marginRight: "5px", fontWeight: "600" }}>
                    {translate("This language is a proto-language")}
                  </label>
                  <input
                    type="checkbox"
                    id="proto"
